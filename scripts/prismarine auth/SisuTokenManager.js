...
async function Sisu_BeginAuth(sisu_token){
...

    {
        ...
        console.log(token);
        token.access_token = 'd=' + token.access_token; // assign preamble
...
async function Sisu_AuthUser(sisu_xbl_tokens){
    ...
    // NOTE: seems like we have to clear device token, as any device token used with another request will result in an unauthorized resposnse code???
    Storage_WipeDeviceToken();
    const device_token = await Storage_GetDeviceToken();
    //  NOTE: access tokens generated by using a refresh token must be denoted with 't=' NOT 'd=' so we literally have to track whether we're using a refreshed token or not!!!!
    // to solve this, we just put 't=' or 'd=' in the token string itself so we dont have to worry about this, note that this could cause problems with trying to interact with other xbox APIs?? (that specifically need an access token, which is like 0 apis)
    
    const payload = {
        "AccessToken":sisu_xbl_tokens.access_token,
        "AppId":clientId,
        "DeviceToken":device_token.Token,
        "Sandbox":"RETAIL",
